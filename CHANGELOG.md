...
- **Theme Readability Improvements (Light + Dark Mode)**: Fixed a long-standing contrast collapse where card surfaces were nearly indistinguishable from the page background in both modes. Light mode: `--card` moved from `99%` to `97%` lightness, `--card-border` from `92%` to `86%`, and `--border` from `89%` to `82%` — cards are now clearly visible as elevated panels on the white page. Dark mode: `--card` raised from `17%` to `22%` lightness (a 12% gap vs. the previous 6%), `--muted` dropped from `17%` to `13%` so muted areas look properly sunken below cards rather than identical to them, `--secondary` raised from `22%` to `28%` for additional elevation depth, `--border` raised from `22%` to `28%` for visible dividers, and `--muted-foreground` lightened from `62%` to `70%` for more comfortable secondary text. The full dark-mode visual hierarchy is now: sidebar (6%) → background (10%) → muted/inset (13%) → card (22%) → secondary (28%).
