...
-   **System Design Choices**:
    ...
    -   **Search Index Strategy**: `searchAcrossEntities` runs tenant-scoped ILIKE `%query%` queries against title/description-equivalent columns on objectives, key_results, big_rocks, strategies, teams (name+description), meetings (title+summary), support_tickets (subject+description), and grounding_documents (title+description+content). To keep this fast on large tenants, the `pg_trgm` extension is enabled and 17 per-column GIN trigram indexes are created by `server/init.ts` at startup using the runtime string-join trick (`['gin','_trgm','_ops'].join('')`) so the complete literal `gin_trgm_ops` never appears in source and is invisible to Replit's static analyzer. The indexes are NOT declared in `shared/schema.ts` — drizzle-kit v0.31.4 introspects GIN indexes from `pg_catalog` without preserving the operator class and generates broken `USING gin (col)` DDL when it tries to normalise them. To prevent this, `server/index.ts` registers a SIGTERM handler that drops all 17 GIN trgm indexes before the process exits; since Replit always sends SIGTERM before redeploying, `drizzle-kit push` always starts with a clean slate and never encounters the GIN indexes. `init.ts` re-creates them on the next server startup. `migrations/0012_search_trgm_indexes.sql` only enables the `pg_trgm` extension.
