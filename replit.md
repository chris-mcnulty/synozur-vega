...
- **Key Features**:
    - **Dashboards**: Company OS, Executive (advanced analytics, velocity projections), and Team Dashboards (weekly execution focus). All three dashboards load via a single combined `GET /api/dashboard/context?scope=company|executive|team&teamId=&quarter=&year=` endpoint (`server/routes-dashboard.ts`) that returns foundation, strategies, teams, objectives, key results, big rocks, big rock task counts, meetings, and (executive-only) check-ins in one round trip — replacing 5–7 separate per-entity queries. Per-entity endpoints remain available for detail pages. Mutations invalidate `/api/dashboard/context` alongside the underlying entity cache keys.
    - **Core Modules**: Foundations (mission, vision, values, ambitions), Strategy (AI drafting, goal alignment), Outcomes (hierarchical OKRs, "big rocks", annual goals with AI suggestions), Focus Rhythm (meeting management, including a full-screen Live Meeting Mode at `/focus-rhythm/:meetingId/live` with per-topic timer, current-topic spotlight, keyboard-driven facilitator controls, quick-capture for decisions/risks/action items, and refresh-resilient persistence via a `live_state` jsonb column on the `meetings` table), Reporting (PDF/PPTX export).
    ...
    - **Soft Delete & Trash/Recovery**: Strategic items (objectives, key results, big rocks, strategies, ambitions) use `deletedAt`/`deletedBy` columns instead of hard deletes. All read queries filter `isNull(deletedAt)`. Cascade soft-delete on parents (objective→key results, strategy→linked items). Admin/owner-only `/trash` page (gated by `MANAGE_TENANT_SETTINGS` or platform admin) with `GET /api/trash` and `POST /api/trash/:type/:id/restore` endpoints. A daily `trash-purge` job permanently deletes items after 30 days.
...
- **System Design Choices**:
    ...
    - **Tenant Data Boundary Protection (Critical)**: Tenant-scoped APIs should use the authenticated tenant context on `req.effectiveTenantId` as the preferred source for data scoping. In the current implementation, the `authWithTenant` middleware chain (`requireAuth` → `loadCurrentUser` → `requireTenantAccess`) also supports resolving tenant context from approved request inputs such as headers and, for some existing routes, route params/query/body values (for example, legacy `/api/okr/*` flows). Those client-supplied tenant identifiers must only be used to select/validate a tenant the authenticated user is already authorized to access, never as an unvalidated override of tenancy. All storage methods that accept a `tenantId` parameter must still enforce database-level filtering with `WHERE tenant_id = ?` to prevent cross-tenant data leakage. For new endpoints, prefer `req.effectiveTenantId`, reject requests when no validated tenant context is available, and do not trust raw client-provided tenant identifiers unless they are first resolved and authorized by the middleware.
    ...
    - **MCP (Model Context Protocol) Server**: Standardized AI assistant integration for compatible AI tools, featuring JWT auth, API key management, OAuth 2.0 authorization server, scope-based permissions, read/write tools for platform data, IP allowlisting, and rate limiting. **Dual Copilot Studio auth paths**: (A) **Entra JWT validation** (`server/mcp/entra-jwt.ts`) — validates Microsoft Entra ID-issued tokens using JWKS, maps `oid`/`tid` claims to Vega users/tenants, recommended for M365 Copilot "Authenticate with Microsoft" mode. Env vars: `MCP_ENTRA_CLIENT_ID` (audience), `MCP_ENTRA_ALLOWED_TENANTS` (optional comma-separated tenant IDs). (B) **Vega OAuth 2.0** — built-in authorization server at `/oauth/authorize`, `/oauth/token`, `/oauth/.well-known/openid-configuration` for Copilot Studio "Manual" OAuth mode with Authorization Code + PKCE flow. OAuth clients managed via `/api/oauth/clients` (admin only). Schema: `oauth_clients`, `oauth_authorization_codes`, `oauth_refresh_tokens` tables. Auth methods: `api_key`, `jwt_token`, `oauth`, `entra_jwt`.
